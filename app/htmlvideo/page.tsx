...
export default function HTMLVideoPage() {
  ...
  // ฟังก์ชัน fallback สำหรับระยะเวลา (อาจใช้วิธีอื่นแทน)
  const getFallbackDuration = async (): Promise<string> => {
    // อาจดึงจากฐานข้อมูลของคุณหรือใช้วิธีอื่น
  ...
  const fetchVideos = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('HTML videos filtered:', htmlVideos);
        
        if (htmlVideos.length === 0) {
          console.log('No HTML videos found');
          setVideos([]);
          setLoading(false);
          return;
        }
        ...
        const videosWithDuration = htmlVideos.map((video: Video) => ({
          ...video,
  ...
  useEffect(() => {
    ...
    if (user) {
      console.log('User found, fetching skipped lessons...');
      fetchSkippedLessons();
    } else {
      console.log('No user found, skipping lesson progress');
    }
    ...
    return (
      <RouteGuard requireAuth={true}>
        <PretestChecker courseType="HTML">
          <div className="min-h-screen bg-emerald-50 flex items-center justify-center">
            <div className="text-center">
              <div className="text-2xl text-gray-700 font-medium mb-4">ไม่พบเนื้อหาวิดีโอ HTML</div>
  ...
  return (
    <RouteGuard requireAuth={true}>
      <PretestChecker courseType="HTML">
        <div className="min-h-screen bg-emerald-50 p-4 flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-96 bg-white rounded-2xl shadow-xl flex flex-col max-h-screen">
            ...
            <div className="p-6 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-t-2xl">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <h3 className="text-xl font-bold mb-1">เนื้อหา HTML</h3>
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredVideos.length === 0 ? (
                <div className="p-6 text-center text-gray-500">
                  ...
                  <p className="font-medium mb-2">ไม่มีเนื้อหาวิดีโอ HTML ที่ต้องเรียน</p>
                  <p className="text-sm">
                    {skippedLessons.length > 0 ? 
            ...
            <div className="p-6 border-t bg-gray-50 rounded-b-2xl">
              <button 
                ...
                className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-4 rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-lg hover:shadow-xl font-medium text-lg flex items-center justify-center"
              >
